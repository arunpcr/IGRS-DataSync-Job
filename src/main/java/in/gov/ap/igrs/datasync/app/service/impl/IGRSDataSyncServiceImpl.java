package in.gov.ap.igrs.datasync.app.service.impl;
...
public class IGRSDataSyncServiceImpl implements IGRSDataSyncService {
	...
	public Map<String, String> dataSyncFromMongoToOracle(String appId) 
	{
		...
		try {
			...
			//ODocumentDetails odd=new ODocumentDetails();
			...
			//OPartyDetails opd=new OPartyDetails();
			...
			//OPartyRepresentDetails oprd=new OPartyRepresentDetails();
			...
			boolean deleteExecutantClaimantFirms=OPartyRepresentDetails.singleDeleteExecutantClaimantFirms(documentId);
			if (deleteExecutantClaimantFirms == false)

				logger.info("ExecutantClaimantFirms record with ID****" + documentId + "***got deleted successsfully");
			...
			for (String mess : PartyRepresentDetails) {
				...
				//firms separate table
				boolean ins_executants_claimant_firms=OPartyRepresentDetails.singleInsertExecutantsClaimantFirms(partyRepresentDetails1);
				if(ins_executants_claimant_firms==false)
				logger.info("Executants_claimant_firms record with ID****" + documentId + "***got inserted successsfully");
				else if (ins_executants_claimant_firms == true) {
					logger.info("Executants_claimant_firms record with ID****" + documentId
							+ "***could not be inserted successsfully");
					status = "FAILED";
				}
...
boolean updateECFirmsDetails=OPartyRepresentDetails.updateECFirmsDetails(partyRepresentDetails1, mongoDBUrl, dbName);
				

				if (updateECFirmsDetails == false)
				   logger.info("PartyRepresent-updateECFirmsDetails record with ID****" + documentId + "***got updated successsfully");
				else
					
					logger.error("PartyRepresent record-updateECFirmsDetails with ID****" + documentId + "***could not be updated successsfully");
				logger.info("************************ijaya Completed**************************");
			...
			//OPropertyDetails opropd=new OPropertyDetails();
			...
			for (int i = 0; i < propertyDetails.size(); i++) {
				...
				//OPropertyStructureDetails opropsd=new OPropertyStructureDetails();
	...
	public Map<String, String> dataSyncFromMongoToOracleProd(String appId) 
	{
		...
		try {
			...
			for (String mess : PartyRepresentDetails) {
				...
				boolean insertPartyRepresentDetails = OPartyRepresentDetails.singleInsertPartyRepresentDetails(partyRepresentDetails1);
